...
$(function() {
    ...
    function utilitiesData(response){
        ...
        $('#utilities_table').grid({
            ...
            columns:[
                ...
                {tmpl: '<input type="number" class="num_input amt">', title: 'Քանակ'},
                {tmpl: '<input type="number" class="num_input price" data-id="{id}" >', title: 'Գումար'},
                {tmpl: '<select class="form-control" id="select_month">'+months+'</select>', title: 'Որ ամսվա համար'},
            ]
        });
    ...
    function expensesData() {
        ...
        let grid = $('#add_expenses_table').grid({
            dataSource: {url: 'user7/add_expenses_data', type: 'post', success: function (response) {
                    let records = [];
            ...
            notFoundText: 'Արդյունք չի գտնվել',
            columns:[
                {field: 'date', title: 'Ապրանքի ընդունման ամսաթիվ'},
                {field: 'balance', title: 'Օրվա գործարքի ընդհանուր գումարը'},
                {tmpl: '<button>Վճարել</button>', align:'right', cssClass: 'fa_button', events: {'click': acceptExpenses}},
        ...
        function acceptExpenses (e){
            ...
            $('#btn_accept_expense').off('click').on('click', function () {
                let date = e.data.record.date;
                let supplier_id = e.data.record.supplier_id;
                $.ajax({
    ...
    function paidExpensesHistory() {
        ...
        let expenses_grid = $('#grid_history_expenses').grid({
            dataSource: {url: 'user7/paid_goods_history', type: 'post', success: function (response) {
                    let records = [];
                    for (let x in response){
                        let sum = 0;
                        for (let j in response[x]){
            ...
            columns: [
                ...
                {field: 'release_date', title: 'Ընդունման ամսաթիվ'},
                {field: 'sum', title: 'Օրվա գործարքի ընհանուր գումարը '},
            ],
        });
        ...
        grid.on('detailExpand', function (e, $detailWrapper, id) {
            let detail = $detailWrapper.find('table').grid({
                ...
                columns: [
                    ...
                    { field: groupBy('product_name','client_name'), title: groupBy('Ապրանք ','Հաճախորդ ')+'<i class="fas fa-sort"></i>', sortable: true },
    ...
    function getClientsInfo() {
        ...
        let grid = $('#client_table').grid({
            dataSource: {url: 'user7/client_data', type: 'post', success: (response)=>{
    ...
    function getGoodsInfo(response) {
        ...
        let grid = $('#good_table').grid({
            ...
            columns: [
                ...
                { field: 'price', title: 'Գին <i class="fas fa-sort"></i>', editor: true, sortable: true },
                { field: 'place_name', title: 'Ուղություն', type: 'dropdown', editField: 'place_id',
        ...
        $('#add_new_good').off('click').on('click', function () {
            ...
            $('#btn_good').off().on('click', function () {
                ...
                let place_id = $('#good_modal_select');
    ...
    function getProductInfo() {
        ...
        $('#btnAddRow').off().on('click', function () {
            ...
            $('.add_new_product_table').append('<tr>\n' +
                ...
                '<td><input type="text" class="gj-textbox-md modal_input new_product_height"></td>' +
        ...
        let grid = $('#product_table').grid({
            ...
            columns: [
                ...
                { field: 'local_price', title: 'Արտահանման գին <i class="fas fa-sort"></i>', editor: true, sortable: true },
    ...
    function getAcceptInfo() {
        ...
        table.grid({
            ...
            columns: [
                ...
                {title: 'Գումար',tmpl: '<input type="number" class="num_input paid" data-id="{id}">'},
                {title: 'Դույլ',tmpl: '<input type="number" class="num_input bucket">'},
                {title: 'Կրիշկա',tmpl: '<input type="number" class="num_input lid">'}
            ]
        });
        $('#btn_accept').off().on('click', function () {
            let input = table.find('input.paid');
            let dialog = $('#accept_dialog');
            let data = [];
            $.each(input, function () {
                let bucket = $(this).closest('tr').find('.bucket');
                let lid = $(this).closest('tr').find('.lid');
